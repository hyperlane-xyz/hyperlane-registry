...
describe('BaseRegistry static methods', () => {
  describe('getWarpDeployArtifactPaths', () => {
    it('returns correct paths with default base path', () => {
      const mockDeployConfig = {
        ethereum: { owner: '0x123' },
        arbitrum: { owner: '0x789' },
      } as Record<string, any>;
      const symbol = 'MOCK';

      const result = BaseRegistry.getWarpDeployArtifactPaths(mockDeployConfig, symbol);

      expect(result).to.have.property('configPath');
      expect(result.configPath).to.equal('./configs/MOCK/arbitrum-ethereum-deploy-config.yaml');
    });

    it('respects custom config base path', () => {
      const mockDeployConfig = {
        ethereum: { owner: '0x123' },
        polygon: { owner: '0x456' },
      } as Record<string, any>;
      ...
      const result = BaseRegistry.getWarpDeployArtifactPaths(
        mockDeployConfig,
        symbol,
        customBasePath,
      );

      expect(result.configPath).to.equal('./custom/path/USDC/ethereum-polygon-deploy-config.yaml');
    });

    it('handles many chains in correct sorted order', () => {
      const mockDeployConfig = {
        zksync: { owner: '0x111' },
        ethereum: { owner: '0x222' },
        arbitrum: { owner: '0x333' },
        polygon: { owner: '0x444' },
        optimism: { owner: '0x555' },
      } as Record<string, any>;
      const symbol = 'TOKEN';

      const result = BaseRegistry.getWarpDeployArtifactPaths(mockDeployConfig, symbol);

      // Chain names should be sorted alphabetically
      expect(result.configPath).to.equal(
        './configs/TOKEN/arbitrum-ethereum-optimism-polygon-zksync-deploy-config.yaml',
      );
    });
  });
});
